...
export const updateColumn = async (
  ...
  const updatedColumn = await column.findByIdAndUpdate(columnId, params, {
    new: true,
  });
  if (emit) {
    socket.emit("columns", {
...
export const deleteColumnByParams = async (
  params: any,
  guid: string,
  initUser: string
) => {
  ...
  for (const onColumn of columns) {
    deletedColumns.push(
      await deleteColumnById(onColumn._id, guid, initUser, false)
    );
  }
  socket.emit("columns", {
    action: "delete",
    users: await boardService.getUserIdsByBoardsIds(
      deletedColumns.map((item) => item.boardId)
    ),
