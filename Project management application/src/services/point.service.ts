...
export const createSetOfPoints = async (
  taskId: string,
  boardId: string,
  newPoints: any[],
  guid: string,
  ...
  for (const onePoint of newPoints) {
    createdPoints.push(
      await createPoint({ ...onePoint, taskId, boardId }, guid, initUser, false)
    );
  }
  socket.emit("points", {
    action: "add",
    users: await boardService.getUserIdsByBoardsIds(
      createdPoints.map((item) => item.boardId)
    ),
...
export const updatePoint = async (
  ...
  const updatedPoint = await point.findByIdAndUpdate(pointId, params, {
    new: true,
  });
  if (emit) {
    socket.emit("points", {
...
export const deletePointById = async (
  pointId: string,
  guid: string,
  initUser: string,
  emit = true,
...
export const deletePointsByParams = async (
  params: any,
  guid: string,
  initUser: string
) => {
  ...
  for (const onPoint of points) {
    deletedPoints.push(
      await deletePointById(onPoint._id, guid, initUser, false)
    );
  }
  socket.emit("points", {
    action: "delete",
    users: await boardService.getUserIdsByBoardsIds(
      deletedPoints.map((item) => item.boardId)
    ),
