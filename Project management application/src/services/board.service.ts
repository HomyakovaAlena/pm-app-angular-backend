...
export const findBoardsByUser = async (userId: string) => {
  ...
  return allBoards.filter(
    (item) => item.owner === userId || item.users.includes(userId)
  );
};
...
export const updateBoard = async (
  ...
  const deletedUsers = oldVersion.users.filter(
    (user: string) => !params.users.includes(user)
  );
  const updatedBoard = await board.findByIdAndUpdate(boardId, params, {
    new: true,
  });
  if (emit) {
    socket.emit("boards", {
...
export const deleteBoardByParams = async (
  params: any,
  guid: string,
  initUser: string
) => {
  ...
  for (const onBoard of boards) {
    deletedBoards.push(
      await deleteBoardById(onBoard._id, guid, initUser, false)
    );
  }
  let users: string[] = [];
  deletedBoards.forEach(
    (board) => (users = [...users, ...board.users, board.owner])
  );
  socket.emit("boards", {
    action: "delete",
...
export const clearUserInBoards = async (
  userId: string,
  guid: string,
  initUser: string
) => {
...
export const getUserIdsByBoardsIds = async (boards: string[]) => {
  const allboards = await board.find({});
  const interestedBoards = allboards.filter((item) =>
    boards.includes(item._id.toString())
  );
  let result: string[] = [];
...
export const getBordsIdsByUserId = async (user: string) => {
  const allboards = await board.find({});
  const interestedBoards = allboards.filter(
    (item) => item._doc.users.includes(user) || item._doc.owner === user
  );
