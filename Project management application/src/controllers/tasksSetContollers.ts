...
export const updateSetOfTask = async (req: Request, res: Response) => {
  ...
  for (const oneTask of tasks) {
    ...
    if (taskError) {
      return res
        .status(400)
        .send(createError(400, "bad request: " + taskError));
    }
  ...
  socket.emit("tasks", {
    ...
    users: await boardService.getUserIdsByBoardsIds(
      updatedTasks.map((item) => item.boardId)
    ),
...
export const findTasks = async (req: Request, res: Response) => {
  ...
  if (ids) {
      ...
      return res.json(
        allTasks.filter((oneTask) => {
          ...
          const users = [
            ...allUsers.filter(
              (user) =>
                user._id === new ObjectId(oneTask.userId) ||
                oneTask.users.includes(user._id)
            ),
          ];
    ...
    return res.json(
      allTasks.filter((oneTask) => boards.includes(oneTask.boardId))
    );
  } else {
