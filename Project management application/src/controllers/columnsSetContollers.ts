...
export const updateSetOfColumns = async (req: Request, res: Response) => {
  ...
  for (const oneColumn of columns) {
    ...
    try {
      updatedColumns.push(
        await columnService.updateColumn(_id, { order }, guid, initUser, false)
      );
    } catch (err) {
  ...
  socket.emit("columns", {
    ...
    users: await boardService.getUserIdsByBoardsIds(
      updatedColumns.map((item) => item.boardId)
    ),
...
export const findColumns = async (req: Request, res: Response) => {
  ...
  if (ids) {
    ...
    const allColumns = await columnService.findColumns({});
    return res.json(
      allColumns.filter((oneColumn) => boards.includes(oneColumn.boardId))
    );
  } else {
...
export const createSetOfColumns = async (req: Request, res: Response) => {
  ...
  socket.emit("columns", {
    action: "add",
    users: await boardService.getUserIdsByBoardsIds(
      createdColumns.map((item) => item.boardId)
    ),
